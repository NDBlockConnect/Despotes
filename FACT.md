...
- **v26.12** stable (LTS): batch control flow + structured errors + MC 1.20.1 support across three loaders + Aprism Loader v26.8 adaptation, hardened by production testing. Shutdown fix: `Despotes.shutdown()` was never invoked, so the JDK httpserver's non-daemon dispatcher thread blocked JVM exit and tripped the vanilla client shutdown watchdog — now registered as a JVM shutdown hook with an idempotency guard. Repository robustness: 12 modules (aprism, native ×6, neoforge ×2, plus Mojang-repo additions to native 1.21.x) declared no `repositories`, resolving only via the machine-local `~/.m2`; all now declare explicit `mavenLocal` + `mavenCentral` (+ Mojang/NeoForged where required). 20/20 artifacts + `.aje` built on the v26.12 stamp.
- **v26.12.1** (Sub-Major patch): **multi-instance attribution** — fixes phantom/wrong answers when several Despotes instances run on one machine. `status` now reports `instanceId` (8-hex SHA-256 of the normalised game directory), `instanceName` (folder name) and `gameDir`, plus a `transports.http` block carrying `configuredPort` / `boundPort` / `listening` / `bindError`. The HTTP transport no longer fails silently on a port clash: it logs the clash together with the instance identity and the remedy. Verified by squatting port 25585 with a foreign responder — a caller can now distinguish `instanceId=deadbeef/OTHER-INSTANCE` from the real instance `c9955944`, where the previous release answered identically for both. Ships 17/20 artifacts (see Known issues for the neoforge 1.21.x network block).
...
### v26.12 robustness assessment (2026-09-17, input for v26.13 planning)
- Setup: MC 26.2 / Fabric Loader 0.19.3, `bct-a262-fab` with the released `Despotes-v26.12-fabric-26.2.jar`; HTTP 25585 + WS 25586, security gate enabled (token not required).
- Simulated usage (title-screen and in-world contexts): 14 query/action probes — `status`/`self`/`world`/`screen`/`target`/`pending`/`container`/`hotbar`/`threats` answered normally; `circuit`/`redstone` returned the correct structured `NOT_IN_GAME` (v26.11 guard); `inventory`/`blocks` returned `TIMEOUT` rather than `NOT_IN_GAME`. All replies kept the stable `ok`/`error.code` envelope; no crash from any query.
- Finding 1 (LOW, consistency): `inventory` and `blocks` answer `TIMEOUT` when queried outside a world, whereas `circuit`/`redstone` answer `NOT_IN_GAME`. Same precondition, two different codes — worth unifying in v26.13 so agents can branch on one code.
- Finding 2 (MEDIUM, documented below): the post-main shutdown watchdog crash is reproducible on demand (`mdl kill` / any external close). Root cause re-characterised with an isolated JDK probe (see Known issues) — the v26.12 shutdown hook is inert by construction; an application-level stop is required.
- Finding 3 (LOW, docs): the batch control flow introduced in v26.12 uses the `{"batch":[...]}` envelope, not `{"type":"batch"}`; a first probe with `type:batch` was answered `UNKNOWN_TYPE` (correct behaviour, but the docs/examples are the only discoverable place — consider an explicit hint in the error message).
- Environmental note: two of three launches died during the mixin/atlas phase because parallel sessions held the host at ~1.3 GB free (a peer instance ran with `-Xmx8022M`); no Despotes exception in any log. Re-runs required `--no-queue` to bypass MDL's global instance lock.
- Assessment: **no Serious-level findings**; protocol layer stable, structured-error contract holds, no crash attributable to Despotes input handling. v26.13 planning inputs: (a) unify not-in-world error codes, (b) application-level shutdown stop, (c) minor protocol hint for unknown batch envelopes.
...
- **neoforge 1.21.x rebuild blocked by network environment (2026-09-17, v26.12.1)**: `piston-meta.mojang.com` is unreachable from this host (TLS handshake reset, direct *and* via the local proxy), so NeoGradle cannot refresh its version manifest. Secondary damage compounds it: partially-downloaded archives leave `zip END header not found` in the isolated Gradle home, and re-seeding the machine-local `~/.m2` re-triggers the NeoGradle 7.1.38 MavenLocal descriptor mutation bug. `neoforge-26.1.2` and `neoforge-26.2` rebuilt fine (their pipeline artifacts were already resident). v26.12.1 therefore ships **17/20** artifacts; `neoforge-1.21.1/1.21.4/1.21.10` remain at their v26.12 builds — they carry the same `common/` code minus this patch's instance-identity fields. Rebuild them when `piston-meta.mojang.com` is reachable again.
- **Post-main shutdown watchdog crash (open, LOW–MEDIUM; re-characterised 2026-09-17)**: the vanilla client shutdown watchdog can still kill the process with `Client shutdown from post-main` when the JVM is asked to exit while Despotes' HTTP transport is listening. Root cause pinned down with an isolated probe: the JDK `HttpServer` spawns a **non-daemon** `HTTP-Dispatcher` thread; `Runtime.addShutdownHook` therefore **never runs** (a hook only executes after all non-daemon threads end), so `Despotes.shutdown()` cannot rescue the exit path — crash-report dumps show `DestroyJavaVM` with no `Despotes-Shutdown` thread. Empirically: the probe hangs with **and** without the hook; calling `server.stop(0)` from a normal thread does terminate the dispatcher (verified), so an application-level stop before MC's post-main watchdog is the viable fix. Planned for v26.13: expose a `shutdownRequested()` probe on `IGamePlatform` (5 loader impls) and call `shutdown()` from `clientTick()` when the client begins closing, or replace the JDK httpserver with a daemon-threaded implementation. Impact: cosmetic crash report + slight exit delay; the game still exits and no world data is lost (MC saves before the watchdog fires).
- **Aprism v26.8 × fabric-hosted launches broken (upstream, found 2026-09-05)**: the refreshed Aprism agent jar bundles ASM 9.9 while MDL's game classpath carries `mdl/libraries org.ow2.asm:asm:9.10.1`; Fabric Loader 0.19.3 `LoaderUtil.verifyClasspath` hard-fails on ANY duplicate ASM source (`Knot.<clinit>` → `ExceptionInInitializerError`). MDL (26.5.0-alpha.7) auto-restores removed libraries, so file-level workarounds do not persist. Verified workarounds: run the Aprism line on a **vanilla-host instance** (`--loader none` + `--aprism`) — the v26.8 agent bootstraps vanilla directly and Despotes .aje loads fully; also note Aprism's manifest resolver is stricter on fabric hosts (modmenu required an unresolvable `fabric-resource-loader-v1`). Needs upstream fix (MDL classpath dedup or Aprism ASM relocation).
  - **RESOLVED (2026-09-16)**: upstream shipped `Aprism-v26.9-Alpha.8-JE-26.2-fabric-host.jar` with relocated ASM (base-asm=0, packages moved to `aprism/libs/asm/`); Fabric Loader 0.19.3 `verifyClasspath` passes and 48 fabric mods load cleanly. Attach recipe via MDL `--javaagent "<jar>=aprismVersion=v26.9-Alpha.8;mcEdit=JE;mcVersion=26.2;gameRoot=<gameRoot>"` (arg format per `AprismAgent.parseArgs`; MDL `--aprism` still auto-attaches the non-relocated v26.8 agent). End-to-end verified: `runtime initialized: v26.9-Alpha.8 / JE / 26.2`, `performLoad complete: 2 mod(s)`, boot report `[OK] mod despotes 26.12.0 (76 ms)` — Despotes `.aje` fully initializes on a fabric host for the first time in the v26.12 era. Vanilla-host (`--loader none` + `--aprism`) remains the zero-args path. Upstream-provided fix; no Despotes code change required.
...
- neoforge-1.21.1 config-time NG bug: NeoGradle 7.1.38's `CommonProjectPlugin` exclusiveContent realization (line ~131-134, lwjgl-freetype filter) mutates all repository descriptors including MavenLocal; if any resolution preceded it in the container, the build dies with "Cannot mutate content repository descriptor MavenLocal after repository has been used". RELIABLE WORKAROUND (v26.12-Alpha.3): run the build under an isolated `GRADLE_USER_HOME` (e.g. workspace `.gradle-hermetic/`) + `JAVA_TOOL_OPTIONS` proxy for central/neoforged maven; transient central TLS resets may require 1-2 retries per module. Additionally, `neoforge-1.21.4`/`1.21.10` settings.gradle carried a copy-pasted rootProject.name (fixed in Alpha.3).
...
- **fabric-26.1.2 event-stream RESOLVED (v26.12-Alpha.3)**: the MessageCaptureMixin was never broken. Root cause of the "silently not applied" diagnosis: test sessions never entered a world, so `ClientPacketListener` never loaded and the mixin never had a chance to apply. In-world verification (v26.12-Alpha.2 artifact, identical code): static-block + ctor-inject logs fire on world join, `handleSystemChat`/`handleDisguisedChat`/`handlePlayerChat` injections publish `system`/`chat` events to the event stream end-to-end (verified via `type` action + `GET /events`). Lesson: event-stream verification MUST include an in-world step; title-screen-only sessions cannot exercise packet-handler mixins.
