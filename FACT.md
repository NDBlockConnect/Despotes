...
- **neoforge-1.21.4 build-environment limitation**: the real neoform 21.4.157 userdev classpath rejects our source (getId/InputConstants/Screen signatures differ from what fabric loom's 1.21.4 mappings provide 鈥?mapping-provider divergence). All shipped neoforge-1.21.4 artifacts (v26.2鈫抳26.11) are compiled against legacy NeoForge 21.1.248 classes shadowing 21.4 ones (historical method, runtime-verified in v26.1-Alpha.4 but compile-time API checks are not strict for this line). Proper 21.4 API adaptation is a v26.12 work item.
