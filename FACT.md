...
- fabric-26.1.2 line: event-stream capture silent — root cause confirmed (see below); chat sending works
  - v26.12 diagnosis: fabric-api 0.155.2 message-api has NO client-side mixins (server-only) — ClientReceiveMessageEvents can never fire. A packet-level MessageCaptureMixin ships but its class never loads under fabric loader 0.19.2 on 26.1.2 (ctor-inject diagnostics confirm; config silently not applied). Next levers: JVM mixin debug flags via custom launch script, refmap/minVersion investigation.
- **neoforge-1.21.4 build-environment limitation**: the real neoform 21.4.157 userdev classpath rejects our source (getId/InputConstants/Screen signatures differ from what fabric loom's 1.21.4 mappings provide — mapping-provider divergence). All shipped neoforge-1.21.4 artifacts (v26.2→v26.11) are compiled against legacy NeoForge 21.1.248 classes shadowing 21.4 ones (historical method, runtime-verified in v26.1-Alpha.4 but compile-time API checks are not strict for this line). Proper 21.4 API adaptation is a v26.12 work item.
- Cross-project FACT audit (2026-08-26) outcomes:
  - Aprism [FINDING] empty MANIFEST.MF → FIXED: aprism jar/.aje now carry Implementation-* metadata (Gradle 9 removed project.manifest/Zip.metaInf — manifest generated as file, placed via from+into).
  - Aprism [BLOCKER] click coordinate space → ADDRESSED: screen query embeds `window` block {physicalWidth, physicalHeight, width, height, guiScale}; conversion formula physical/guiScale. Verified 854x480 / 427x240 / scale 2.
  - Refact report "process vanished after death" → LifeCycleMonitor audited: publishes events only, no process control. Death-time disappearance attributed to mdl idle-timeout on quiet death screens.
  - Prismate blocked-on-tooling → RESOLVED: fabric-26.2 artifacts exist since v26.10.
